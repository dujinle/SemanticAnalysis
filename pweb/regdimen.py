...
class RegDimenHandler(RequestHandler):
	...
	def post(self):
		...
		label = self.body_json['label'];
		if len(itext) == 0:
			self.execpt_handle('the param text is empty');
			return ;
		logging.info('%s\tscene:%s itext:%s' %(__file__,label,itext));
		try:
			self.menj.reg_dimen(label,itext);
		except Exception,e:
			self.except_handle(format(e));
			return ;
		self.write(self.gen_result(0,'register dimen words success',None));

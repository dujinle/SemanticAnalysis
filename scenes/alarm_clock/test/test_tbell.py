...
def analysis_result(struct,ans):
	...
	if not clock.has_key('prompt'): return False;
	if clock['prompt']['type'] == ans['prompt']: return True;
	return False;
...
struct['result'] = dict();
#init data
ck_list = common.read_json('./mtbell.init');
for ck in ck_list:
	se.clocks[ck['key']] = ck;
se.myclock = ck_list[0];

tests = common.read_json('./mtbell.test');
for test in tests:
	#start tests
